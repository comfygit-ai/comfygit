...
class GlobalCommands:
    ...
    @with_workspace_logging("config torch-backend show")
    def config_torch_show(self, args: argparse.Namespace) -> None:
        """Show current PyTorch backend setting."""
        from comfygit_core.managers.pytorch_backend_manager import PyTorchBackendManager

        # Get the active environment to show its backend
        env = self.workspace.get_active_environment()
        if not env:
            print("✗ No active environment. Use: cg use <name>")
            print("   Or specify with: cg -e <name> config torch-backend show")
            sys.exit(1)

        # Read backend from file or detect
    ...
    @with_workspace_logging("config torch-backend set")
    def config_torch_set(self, args: argparse.Namespace) -> None:
        """Set PyTorch backend for active environment."""
        from comfygit_core.managers.pytorch_backend_manager import PyTorchBackendManager

        env = self.workspace.get_active_environment()
        if not env:
            print("✗ No active environment. Use: cg use <name>")
            print("   Or specify with: cg -e <name> config torch-backend set <backend>")
            sys.exit(1)
    ...
    @with_workspace_logging("config torch-backend detect")
    def config_torch_detect(self, args: argparse.Namespace) -> None:
        ...
        if not env:
            print("✗ No active environment. Use: cg use <name>")
            print("   Or specify with: cg -e <name> config torch-backend detect")
            sys.exit(1)
        ...
        if detected != current:
            print()
            print(f"💡 Consider updating: cg config torch-backend set {detected}")
