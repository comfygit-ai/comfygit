...
class TestDevNodeRequirementsAutoRefresh:
    """Dev node requirements should be auto-refreshed on sync."""

    def test_sync_updates_changed_dev_node_requirements(self, test_env):
        """sync() should detect and update changed dev node requirements."""
        # ARRANGE - Create dev node with requirements
        dev_node_path = test_env.comfyui_path / "custom_nodes" / "reqs-dev-node"
        dev_node_path.mkdir(parents=True)
        (dev_node_path / "nodes.py").write_text("# dev node code")
        (dev_node_path / "requirements.txt").write_text("numpy>=1.0\n")

        test_env.node_manager.add_node("reqs-dev-node", is_development=True)

        # Verify initial requirements captured
        config = test_env.pyproject.load(force_reload=True)
        groups = config.get('dependency-groups', {})
        # Find the group for this node
        group_name = None
        for name in groups:
            if 'reqs-dev-node' in name:
                group_name = name
                break
        assert group_name is not None, "Dependency group should exist for dev node"
        initial_reqs = groups[group_name]
        assert any('numpy' in str(r) for r in initial_reqs), "numpy should be in initial requirements"

        # CHANGE requirements
        (dev_node_path / "requirements.txt").write_text("pandas>=2.0\nscipy>=1.0\n")

        # ACT - Run sync
        test_env.sync(dry_run=True)  # dry_run to avoid actual uv operations

        # ASSERT - Requirements should be updated
        config = test_env.pyproject.load(force_reload=True)
        groups = config.get('dependency-groups', {})
        updated_reqs = groups.get(group_name, [])

        # Should have new requirements
        req_str = ' '.join(str(r) for r in updated_reqs)
        assert 'pandas' in req_str, "pandas should be in updated requirements"
        assert 'scipy' in req_str, "scipy should be in updated requirements"
        # Old requirement should be gone
        assert 'numpy' not in req_str, "numpy should no longer be in requirements"

    def test_sync_removes_empty_dev_node_requirements_group(self, test_env):
        """sync() should remove dependency group when dev node has no requirements."""
        # ARRANGE - Create dev node with requirements, then remove them
        dev_node_path = test_env.comfyui_path / "custom_nodes" / "empty-reqs-node"
        dev_node_path.mkdir(parents=True)
        (dev_node_path / "nodes.py").write_text("# dev node code")
        (dev_node_path / "requirements.txt").write_text("numpy>=1.0\n")

        test_env.node_manager.add_node("empty-reqs-node", is_development=True)

        # Remove requirements
        (dev_node_path / "requirements.txt").write_text("")

        # ACT
        test_env.sync(dry_run=True)

        # ASSERT - Dependency group should be removed
        config = test_env.pyproject.load(force_reload=True)
        groups = config.get('dependency-groups', {})

        # No group should reference this node
        for name in groups:
            assert 'empty-reqs-node' not in name, (
                f"Empty requirements group should be removed, found: {name}"
            )

    def test_sync_skips_unchanged_dev_node_requirements(self, test_env):
        """sync() should not modify pyproject if dev node requirements unchanged."""
        # ARRANGE
        dev_node_path = test_env.comfyui_path / "custom_nodes" / "unchanged-node"
        dev_node_path.mkdir(parents=True)
        (dev_node_path / "nodes.py").write_text("# dev node code")
        (dev_node_path / "requirements.txt").write_text("requests>=2.0\n")

        test_env.node_manager.add_node("unchanged-node", is_development=True)

        # Get initial pyproject state
        initial_config = test_env.pyproject.load(force_reload=True)

        # ACT - Run sync without changing requirements
        test_env.sync(dry_run=True)

        # ASSERT - Pyproject should be unchanged
        final_config = test_env.pyproject.load(force_reload=True)

        # Find the group
        for name in initial_config.get('dependency-groups', {}):
            if 'unchanged-node' in name:
                assert initial_config['dependency-groups'][name] == final_config['dependency-groups'].get(name), (
                    "Unchanged requirements should not modify pyproject"
                )
                break

    def test_sync_handles_missing_dev_node_directory(self, test_env):
        """sync() should handle dev nodes whose directories are missing."""
        # ARRANGE - Track a dev node, then delete its directory
        dev_node_path = test_env.comfyui_path / "custom_nodes" / "deleted-dev-node"
        dev_node_path.mkdir(parents=True)
        (dev_node_path / "nodes.py").write_text("# dev node code")

        test_env.node_manager.add_node("deleted-dev-node", is_development=True)

        # Delete the directory
        import shutil
        shutil.rmtree(dev_node_path)

        # ACT - sync should not crash
        result = test_env.sync(dry_run=True)

        # ASSERT - Sync should complete (missing node reported separately)
